...
def curate(sel, comps, selected, rows, ind_ppu):
    """The outgoing comp list (client export AND underwriting data): the
    grid's 5 comps first, then only comps priced within 1 SD (of the grid
    comps' $/unit) of the grid's indicated value — nothing that reads too
    low or too high against the number we're quoting. If more than 20 rows
    survive, trim to the top 15 by rank."""
    import statistics
    ...
    print(f"curated comp list: {len(pool)} comps "
          f"({screened} screened out as >1 SD from indicated ${ind_ppu:,.0f}/unit, "
          f"band ±${sd:,.0f})")
    return pool


def build_export_sheet(ws, sel, pool, selected, st):
    s = sel["subject"]
    sel_ranks = set(sel["selected_ranks"])
...
def build_underwriting_wb(sel, comps, path):
    """Underwriting Sale Data: the legacy Output Analysis Data schema over the
    same curated pool as the client export (grid 5 first, ±1 SD screen,
    15-row cap), plain table (feeds the underwriting model)."""
    st = styles()
...
def build_xlsx(sel, pool, selected, rows, cap, path):
    st = styles()
    wb = openpyxl.Workbook()
    build_export_sheet(wb.active, sel, pool, selected, st)
...
def main():
    ...
    pool = curate(sel, comps, selected, rows, ind_ppu)
    build_xlsx(sel, pool, selected, rows, cap, xlsx)
    build_underwriting_wb(sel, pool, uw)
    build_pdf(sel, rows, ind_ppu, ind_total, cap, pdf)
    print(f"Wrote {xlsx} (client-facing, {len(pool)} comps)")
    print(f"Wrote {uw} (same curated {len(pool)} comps, legacy schema)")
    print(f"Wrote {pdf}")
