...
def main():
    ...
    check("underwriting file carries the same curated pool as the client export",
          uws.max_row - 1 == n_rows, f"{uws.max_row-1} vs client {n_rows}")
    check("underwriting grid-5 first and list capped like the client export",
          [uws.cell(r, 1).value for r in range(2, 7)] == top5 and uws.max_row - 1 <= 20,
          f"top5={[uws.cell(r,1).value for r in range(2,7)]}, rows={uws.max_row-1}")
