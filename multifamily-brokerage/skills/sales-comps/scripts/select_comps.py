#!/usr/bin/env python3
"""Score, filter, and select sale comps from an Automatic CMA Analysis workbook.

Reads the 'All Sale Comps' tab (comps universe, with lat/lon) and the
'AgencyDrift' tab (Combined Fannie & Freddie loan data, for cap-rate drift),
scores every comp against the subject, trims $/unit outliers from the best 10,
and writes a selection.json consumed by build_output.py.

The scoring replicates the workbook's original Power Query logic
(distance/age/date point brackets) and adds a unit-count component so all four
criteria are blended. See references/scoring.md for the rationale.
"""
import argparse
import json
import math
...
def read_sheet(path, title):
    ...
    if title not in wb.sheetnames:
        sys.exit(f"ERROR: sheet '{title}' not found in {path} (has: {wb.sheetnames})")
    ws = wb[title]
...
def main():
    ...
    ap.add_argument("--cma", required=True, help="Automatic CMA Analysis .xlsx")
    ...
    ap.add_argument("--fannie-freddie", help="optional standalone Combined Fannie & Freddie xlsx (Sheet1) to use instead of the CMA's AgencyDrift tab")
    ap.add_argument("--comps-sheet", default="All Sale Comps")
    ...
    # ---- comps universe -----------------------------------------------------
    raw = read_sheet(args.cma, args.comps_sheet)
    ...
    if args.fannie_freddie:
        drift_rows = read_sheet(args.fannie_freddie, "Sheet1")
    else:
        drift_rows = read_sheet(args.cma, "AgencyDrift")
    ...
    result = {
        ...
        "comps": comps[:50],   # top 50 go into the Output Analysis Data tab
