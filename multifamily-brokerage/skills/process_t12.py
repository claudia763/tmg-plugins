...
def parse_t12_toki_xlsx(path, trust_monthly=False, allow_partial=False):
            ...
            if best is None:
                ...
                if memo_names and not with_memo:
                    print("!! MEMO ROW(S) EXCLUDED FROM REVENUE: "
                          + "; ".join(memo_names)
                          + " - printed above the income section and proved "
                            "month-by-month to be OUTSIDE the statement's own "
                            "Total Operating Income (gross-potential-rent "
                            "reference). Counting it as revenue would "
                            "double-count rent.")
    ...
    for it in memo_rows:
        ...
        note_line.append(
            ...
            f"{nm} reported months. The statement prints it ABOVE the income "
            f"section and excludes it from its own Total Operating Income "
            f"(verified month by month), so it is carried as a memo reference "
            f"only and is NOT counted as revenue - including it would "
            f"double-count rent against Rent Income.")
    note_line.append(
        "The statement's own \"Total Operating Expense\" row rolls up "
        "UTILITY + Wages only; Tax, Insurance and the Maintenance section sit "
        "outside it. Total Operating Expenses here is the statement's printed "
        "Total Operating Income less its printed NOI, proved equal to the sum "
        "of every expense line, month by month.")
...
def main(argv=None):
    ...
    if _is_owner_rental_schedule(args.pdf):
        ...
        # xlsx parsers return (prop, months, lines); the ones that have
        # something to declare about the source (memo rows, empty month
        # columns) return a fourth `meta` dict on the same contract the PDF
        # parsers use.
        res = parse_t12_xlsx(
            args.pdf, trust_monthly=args.trust_monthly,
            allow_partial=args.allow_partial)
        prop, months, lines = res[:3]
        meta = res[3] if len(res) > 3 else None
    ...
    if args.pad_to_12 and n_mo < 12:
        ...
        # The reported months must be an ordered SUBSET of that axis. A short
        # tail (the common case) is contiguous; a statement stitched from two
        # part-year exports can also be missing a month in the MIDDLE (Werner
        # Creek: Dec-2025 was never produced), which is the same problem and
        # gets the same answer - a dated column with genuinely empty cells.
        akeys = [(d.year, d.month) for d in axis]
        mkeys = [(d.year, d.month) for d in months]
        if sorted(set(mkeys)) != mkeys or not set(mkeys) <= set(akeys):
            sys.exit("ERROR: --pad-to-12 needs the reported months to be an "
                     "ordered subset of the trailing-12 axis ending at the "
                     f"last reported month ({axis[0]:%b %Y}-{axis[-1]:%b %Y})"
                     f"; got {[f'{d:%b %Y}' for d in months]}.")
        real_idx = [akeys.index(k) for k in mkeys]
        pad = [axis[i] for i in range(12) if i not in real_idx]
        months = axis
        n_mo = 12
        gaps, run = [], [pad[0]]
        for d in pad[1:]:
            prev = run[-1]
            if (d.year, d.month) == (prev.year + (prev.month == 12),
                                     prev.month % 12 + 1):
                run.append(d)
            else:
                gaps.append(run)
        ...
        gaps.append(run)
        pad_note = (", ".join(
            f"{g[0]:%b %Y}" if len(g) == 1 else f"{g[0]:%b %Y} - {g[-1]:%b %Y}"
            for g in gaps)
            + (" not provided by ownership - column"
               + ("s" if len(pad) > 1 else "")
               + " intentionally blank (never zero, never annualized)."))
        print(f"!! --pad-to-12: trailing-12 axis {axis[0]:%b %Y}-"
              f"{axis[-1]:%b %Y}; {len(pad)} month(s) "
              f"({', '.join(f'{d:%b %Y}' for d in pad)}) have NO data and are written "
